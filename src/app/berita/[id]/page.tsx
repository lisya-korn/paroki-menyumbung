        ...
        <div className="article-layout">
          ...
          <aside className="article-sidebar">
            <div className="sidebar-card">
              <h3 className="sidebar-card__title">Berita Lainnya</h3>
              <div className="sidebar-card__body">
                <div className="sidebar-list">
                  {beritaLainnya.map((item: any) => {
                    const thumb = item.imageUrl ? item.imageUrl.split('\n')[0] : null;
                    return (
                      <Link href={`/berita/${item.id}`} key={item.id} className="sidebar-news">
                  ...
                  {beritaLainnya.length === 0 && <p style={{ padding: '0 16px', fontSize: '0.9rem', color: '#6B7280' }}>Belum ada berita lain.</p>}
                </div>
              </div>
            </div>
          </aside>
