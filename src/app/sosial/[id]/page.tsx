...
export default async function SosialDetail({ params }: { params: Promise<{ id: string }> }) {
        ...
        <div className="article-layout">
          <div className="article-main">
            ...
            {images.length > 0 ? (
              ...
              <div className="article-image">
                <div style={{ width: '100%', height: '400px', background: 'linear-gradient(135deg, #86C5FF, #2E5AA7)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.3)', fontSize: '4rem', borderRadius: '12px' }}>
                  📷
                </div>
              </div>
            )}
          ...
          <aside className="article-sidebar">
            <div className="sidebar-card">
              <h3 className="sidebar-card__title">Kegiatan Sosial Lainnya</h3>
              <div className="sidebar-card__body">
                <div className="sidebar-list">
                  {beritaLainnya.map((item: any) => {
                    const thumb = item.imageUrl ? item.imageUrl.split('\n')[0] : null;
                    return (
                      <Link href={`/sosial/${item.id}`} key={item.id} className="sidebar-news">
                        <div className="sidebar-news__thumb">
                          {thumb ? (
                            <img src={thumb} alt={item.title} />
                          ) : (
                            <div style={{ background: 'linear-gradient(135deg, #86C5FF, #2E5AA7)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '1.2rem' }}>🤝</div>
                          )}
                        </div>
