              ...
              <div className="sidebar-card__body">
                <div className="sidebar-list">
                  {beritaLainnya.map((item: any) => {
                    ...
                    return (
                      <Link href={`/budaya/${item.id}`} key={item.id} className="sidebar-news">
                        <div className="sidebar-news__thumb">
                          {thumb ? (
                            <img src={thumb} alt={item.title} />
                          ) : (
                            <div style={{ background: 'linear-gradient(135deg, #FFA62B, #F8E6A0)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '1.2rem' }}>🎭</div>
                          )}
                        </div>
