...
export default async function KegiatanImanDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!post || post.category !== 'kegiatan-iman') {
    notFound();
  }
        ...
        <div className="article-layout">
          <div className="article-main">
            <div className="article-breadcrumb">
              ...
              <Link href="/kegiatan-iman">Kegiatan Iman</Link>
              <span className="article-breadcrumb__separator">/</span>
              <span>Detail</span>
            </div>
            ...
            {images.length > 0 ? (
              ...
              <div className="article-image">
                <div style={{ width: '100%', height: '400px', background: 'linear-gradient(135deg, #2E5AA7, #86C5FF)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.3)', fontSize: '4rem', borderRadius: '12px' }}>
                  📷
                </div>
              </div>
            )}
          ...
          <aside className="article-sidebar">
            <div className="sidebar-card">
              <h3 className="sidebar-card__title">Kegiatan Lainnya</h3>
              <div className="sidebar-card__body">
                <div className="sidebar-list">
                  {beritaLainnya.map((item: any) => {
                    const thumb = item.imageUrl ? item.imageUrl.split('\n')[0] : null;
                    return (
                      <Link href={`/kegiatan-iman/${item.id}`} key={item.id} className="sidebar-news">
                        <div className="sidebar-news__thumb">
                          {thumb ? (
                            <img src={thumb} alt={item.title} />
                          ) : (
                            <div style={{ background: 'linear-gradient(135deg, #2E5AA7, #86C5FF)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '1.2rem' }}>⛪</div>
                          )}
                        </div>
                  ...
                  {beritaLainnya.length === 0 && <p style={{ padding: '0 16px', fontSize: '0.9rem', color: '#6B7280' }}>Belum ada kegiatan lain.</p>}
                </div>
              </div>
            </div>
          </aside>
